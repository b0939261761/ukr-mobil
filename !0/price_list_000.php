  ...
  private function loopCategory ($sheet, $categories, $iRow) {
    ...
    foreach ($categories as $category) {
      ...
      if (count($products)) {
        ...
        // категорію, розділ, підрозділ, і додати бренд, якщо є. Якщо немає - то пусто

        $sheet->getCellByColumnAndRow(1, $iRow)->setValue('КАТЕГОРІЯ');
        $sheet->getCellByColumnAndRow(2, $iRow)->setValue('РОЗДІЛ');
        $sheet->getCellByColumnAndRow(3, $iRow)->setValue('ПІДРОЗДІЛ');
        $sheet->getCellByColumnAndRow(4, $iRow)->setValue('БРЕНД');
        $sheet->getCellByColumnAndRow(5, $iRow)->setValue('CСЫЛКА');
        $sheet->getCellByColumnAndRow(6, $iRow)->setValue('КОД ТОВАРА');
        $sheet->getCellByColumnAndRow(7, $iRow)->setValue('НАЗВА');
        $sheet->getCellByColumnAndRow(8, $iRow)->setValue('ЗАЛИШОК Чернівці');
        $sheet->getCellByColumnAndRow(9, $iRow)->setValue('ЗАЛИШОК Рівне');
        $sheet->getCellByColumnAndRow(10, $iRow)->setValue('Ціна, USD');
        $sheet->getCellByColumnAndRow(11, $iRow)->setValue('ЗАМОВЛЕННЯ');
