...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			//region Egofoxlab
			if ($this->user->hasPermission('access', 'ego/post')) {
				$data['menus'][] = [
					'id' => 'ego-posts',
					'icon' => 'fa-cubes',
					'name' => 'Ego',
      ...
      if ($this->user->hasPermission('access', 'design/seo_url')) {
        $design[] = [
          ...
          'href'     => $this->url->link('design/seo_url', ['user_token' => $userToken]),
          'children' => []
        ];
      }
      ...
      if ($this->user->hasPermission('access', 'design/seo_filters')) {
        $design[] = [
          'name'     => 'SEO Фильтр',
          'href'     => $this->url->link('design/seo_filters', ['user_token' => $userToken]),
          'children' => []
        ];
      }
