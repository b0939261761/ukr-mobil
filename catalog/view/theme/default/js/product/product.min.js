...
window.addEventListener('load', function () {
  ...
  const onClickReviewPagination = evt => {
    if (!evt.target.tagName === 'A'
      || !evt.target.parentElement.parentElement.classList.contains('pagination')) return;
    evt.preventDefault();
    setReview(evt.target.href);
  }

  const review = document.getElementById('review');
  review.addEventListener('click', onClickReviewPagination);

  const setReview = async url => {
    const response = await fetch(url);
    review.innerHTML = await response.text();
  }

  setReview(`/index.php?route=product/product/review&productId=${productId}`);
