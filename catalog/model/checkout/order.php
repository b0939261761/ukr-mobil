...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', COMMENT = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', date_added = NOW(), date_modified = NOW()");
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'stock_id' => (int)$order_query->row['stock_id']
			);
		} else {
			return false;
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		$stockId = (int)$order_info['stock_id'];
		...
		if ($order_info) {
			...
			//print_r(array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')));
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) &&
				...
				foreach ($order_products as $order_product) {
					$productId = (int)$order_product['product_id'];
					$productQuantity = (int)$order_product['quantity'];
					$productToStockRow = $productToStockModel->getByProductAndStock($productId, $stockId, true);

					//	Main stock
					if ($stockId === 0 || empty($productToStockRow)) {
						$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . $productQuantity . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
					} //	Additional stock
					else {
						if (!$productToStockModel->setQuantity(
							$productToStockRow->setQuantity($productToStockRow->getQuantity() - $productQuantity)
						)) {
							throw new \RuntimeException("Error occurred while update product quantity.");
						}
					}
