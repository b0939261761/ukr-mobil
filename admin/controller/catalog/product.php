...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$productId = (int)$this->model_catalog_product->addProduct($this->request->post);
			...
			if ($productId > 0) {
				...
				$stockList = Util::getArrItem($this->request->post, 'stock', []);

				foreach ($stockList as $stockId => $quantity) {
					$stockId = (int)$stockId;
					$quantity = (int)$quantity;

					$productToStockRow = (new \Ego\Struct\ProductToStockRowStruct())
						->setProductId($productId)
						->setStockId($stockId)
						->setQuantity($quantity);

					$productToStockModel->setQuantity($productToStockRow);
				}
			}
			//endregion
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//region Egofoxlab
			$productId = (int)Util::getArrItem($this->request->get, 'product_id');
	...
	protected function getList() {
		...
		//region Define Models
		$productToStocksModel = new \Ego\Models\ProductToStock();
		//endregion
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'special'    => $special,
				'quantity'   => (int)$result['quantity'] + $productToStocksModel->getCount((int)$result['product_id']),
...
if (isset($this->request->post['main_category_id'])) {
		...
		//region Egofoxlab
		$productId = (int)Util::getArrItem($_GET, 'product_id');

		//region Define Models
		$stockModel = new \Ego\Models\Stocks();
		$productToStockModel = new \Ego\Models\ProductToStock();
		//endregion

		$data['stockList'] = [];
		$stockList = $stockModel->getList(true);
		...
		foreach ($stockList as $stock) {
			...
			if ($productId > 0) {
				$productToStockRow = $productToStockModel->getByProductAndStock($productId, $stock->getStockId(), true);

				if (!empty($productToStockRow)) {
					$tmp['quantity'] = $productToStockRow->getQuantity();
				}
			}
