	...
	*/
	...
	public function translit($string, $setting) {
		...
		foreach ($custom_replace_from_array as $key => $value) {
			// some values was changed by htmlspecialchars in time of saving $this->request-post!
		...
		// htmlspecialchars
		$string = str_replace(array('&nbsp;', '&quote', '&lt;', '&gt;', '&amp;'), ' ', $string);
		...
		if (is_file($inc_file)) {
			...
			if (is_callable($translit_function)) {
				...
				$this->log->write('ERROR -- SEO URL Generator: function  ' . $translit_function . '() is not calable'
					. ' admin/model/extension/module/seo_url_generator.php on line ' . ( __LINE__ - 4) . '. Code: if (is_callable($translit_function)) {');
			}
		} else {
