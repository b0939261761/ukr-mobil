...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			$product_query->row['quantity'] += $productToStocksModel->getCount((int)$product_query->row['product_id']);
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				//  Check Special price
				$productSpecial = $egoProductModel->getStock((int)$cart['product_id'], (int)$this->customer->getGroupId(), true);

				if (!empty($productSpecial) &&
					time() > strtotime($productSpecial->getDateStart()) &&
					time() < strtotime($productSpecial->getDateEnd())
				) {
					$price = $productSpecial->getPrice();
				}
